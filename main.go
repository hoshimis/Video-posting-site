...
func group(w http.ResponseWriter, rq *http.Request) {
	...
	if rq.Method == "POST" {
		...
		ad = strings.TrimSpace(ad)
		if strings.HasPrefix(ad, "https://youtu.be/") {
			ad = strings.TrimPrefix(ad, "http://youtu.be/")
		}
...
func signup(w http.ResponseWriter, rq *http.Request) {
	...
	item := struct {
		...
		Title:   "Sign Up",
		Message: "type your account & username & password:",
		Account: "",
	}
	...
	if rq.Method == "POST" {
		...
		ur := my.User{
			...
			Name:     rq.FormValue("UserName"),
			Password: rq.FormValue("pass"),
			Message:  "Registration from general users",
		}
